...
donnees['ADX'] = ta.ADX(donnees['High'],
                        ...
                        donnees['Close'],
                        timeperiod=14)
donnees['MACD'], donnees['MACDsignal'], donnees['MACDhist'] = ta.MACD(donnees['Close'], fastperiod=12, slowperiod=26, signalperiod=9)
donnees['ADXR'] = ta.ADXR(donnees['High'], donnees['Low'], donnees['Close'], timeperiod=14)
donnees['STOCHF_fastk'], donnees['STOCHF_fastd'] = ta.STOCHF(donnees['High'], donnees['Low'], donnees['Close'], fastk_period=5, fastd_period=3, fastd_matype=0)
donnees['STOCHRSI_fastk'], donnees['STOCHRSI_fastd'] = ta.STOCHRSI(donnees['Close'], timeperiod=14, fastk_period=5, fastd_period=3, fastd_matype=0)
donnees['APO'] = ta.APO(donnees['Close'], fastperiod=12, slowperiod=26, matype=0)
donnees['PPO'] = ta.PPO(donnees['Close'], fastperiod=12, slowperiod=26, matype=0)
...
donnees['WILLR'] = ta.WILLR(donnees['High'], donnees['Low'], donnees['Close'], timeperiod=14)
donnees['CCI'] = ta.CCI(donnees['High'], donnees['Low'], donnees['Close'], timeperiod=14)
donnees['ATR'] = ta.ATR(donnees['High'], donnees['Low'], donnees['Close'], timeperiod=14)
donnees['NATR'] = ta.NATRdonnees['NATR'] = ta.NATR(donnees['High'], donnees['Low'], donnees['Close'], timeperiod=14)
donnees['TRANGE'] = ta.TRANGE(donnees['High'], donnees['Low'], donnees['Close'])
...
donnees = donnees[[
    'Open', 'High', 'Low', 'Close', 'Volume', 'SMA', 'RSI', 'ADX', 'MACD', 'MACDsignal', 'MACDhist', 'ADXR', 'STOCHF_fastk', 'STOCHF_fastd', 'STOCHRSI_fastk', 'STOCHRSI_fastd', 'APO', 'PPO', 'MOM', 'ROC', 'ROCR', 'WILLR', 'CCI', 'ATR', 'NATR', 'TRANGE'
]]
